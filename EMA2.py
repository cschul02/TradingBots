...
# NEW FIXED POINT RISK MANAGEMENT PARAMETERS
STOP_LOSS_POINTS = 20.0  # Stop Loss distance in index points (e.g., NQ points)
TAKE_PROFIT_POINTS = 50.0 # Take Profit distance in index points (e.g., NQ points)

# Profit Trailing Logic
MOVE_SL_PROFIT_POINTS = TAKE_PROFIT_POINTS / 2.0  # 250 points (Halfway to TP)
NEW_SL_PROFIT_POINTS = TAKE_PROFIT_POINTS * 0.25   # 125 points (25% of TP locked in)
...
def run_backtest(df):
    """
    Runs the backtest strategy with fixed point SL/TP and a trailing mechanism,
    ...
    for index, row in df.iterrows():
        ...
        if position != 0:
            ...
            # A. Trailing Stop Loss Logic (Move SL to lock in 125 points profit)
            if not sl_moved:
                if position == 1:
                    # Long: Check if price moved 250 points up (MOVE_SL_PROFIT_POINTS)
                    if current_close >= entry_price + MOVE_SL_PROFIT_POINTS:
                        # Move SL to entry + 125 points (NEW_SL_PROFIT_POINTS)
                        sl_price = entry_price + NEW_SL_PROFIT_POINTS
                        sl_moved = True
                
                elif position == -1:
                    # Short: Check if price moved 250 points down (MOVE_SL_PROFIT_POINTS)
                    if current_close <= entry_price - MOVE_SL_PROFIT_POINTS:
                        # Move SL to entry - 125 points (NEW_SL_PROFIT_POINTS)
                        sl_price = entry_price - NEW_SL_PROFIT_POINTS
                        sl_moved = True
        ...
        if position == 0 and trades_today < MAX_DAILY_TRADES:
            ...
            if is_bullish_stack:
                ...
                entry_date = index
                sl_price = entry_price - STOP_LOSS_POINTS       # Fixed 200 point SL
                tp_price = entry_price + TAKE_PROFIT_POINTS      # Fixed 500 point TP
                entry_executed = True
...
def analyze_performance(trade_df):
    ...
    print(f"Strategy: Fixed Point SL/TP with Profit Protection")
    print("-" * 70)
    print(f"Risk/Reward: SL {STOP_LOSS_POINTS} points | TP {TAKE_PROFIT_POINTS} points")
    print(f"Profit Protection: Move SL to +{NEW_SL_PROFIT_POINTS} points when +{MOVE_SL_PROFIT_POINTS} is reached.")
